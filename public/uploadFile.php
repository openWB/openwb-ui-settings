...
$GPG_KEY_FILE = "/home/openwb/backup.key";
...
function check_restore_file_contents($path) {
	$cmd1 = "tar --list --file=" . escapeshellarg($path) . " | grep -c '^(mosquitto/|mosquitto_local/|GIT_HASH|GIT_BRANCH|SHA256SUM|backup.log)$'";
	// Wegen grep -E Syntax (POSIX) besser so:
...
function decrypt_gpg($srcPath, $passphrase) {
	...
	$cmd = "gpg --batch --yes --pinentry-mode=loopback --passphrase-fd 0 -o " . escapeshellarg($dest) . " -d " . escapeshellarg($srcPath) . " 2>&1";
	$descriptors = [
		0 => ["pipe","w"],
		1 => ["pipe","r"],
		2 => ["pipe","r"],
	...
	switch ($_POST["target"]) {
		case 'restore':
			...
			if (is_gpg_file($uploaded_tmp)) {
				$is_gpg = true;
				$passphrase = get_gpg_passphrase($GPG_KEY_FILE);
