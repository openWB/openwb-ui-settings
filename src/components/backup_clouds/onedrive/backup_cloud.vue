<template>
	<div class="backup-cloud-one-drive">
		<openwb-base-alert subtype="info">
			...
			dann die Schritte 1-4 durchführen und danach die Konfiguration
			erneut speichern. <br />
			Auth Code und URL werden nur einmal benötigt werden nach
			erfolgreicher Autorisierung wieder gelöscht.
		</openwb-base-alert>
		...
		>
			<template #help>
				In diesem Verzeichnis werden die Backupdateien erstellt. Bitte
				den Pfad mit / beenden. <br />
		...
		<openwb-base-text-input
			...
			readonly
			:model-value="
				backupCloud.configuration.persistent_tokencache ? 'Ja' : 'Nein'
			"
		/>
		<openwb-base-heading>
			Zugang zu OneDrive für diese openWB autorisieren
		</openwb-base-heading>
		<openwb-base-button-input
			title="1. Anmeldeanforderung erstellen"
			buttonText="Autorisierungs-Code anfordern"
			subtype="success"
			@buttonClicked="sendSystemCommand('requestMSALAuthCode', {})"
		>
			<template #help>
				Es werden Zugangstokens für OneDrive angefordert. Diese werden
				nur lokal auf dieser openWB gespeichert. Genutzt wird Device
				Flow mit public application.<br />
				Weitere Infos:<br />
				<a
		...
		>
			<template #help>
				Diese URL im Browser öffnen und den Code eingeben. Dies erzeugt
				die notwendigen Zugangsberechtigung für diese openWB.
			</template>
		</openwb-base-text-input>
		<openwb-base-button-input
			title="4. Token abrufen und speichern"
			buttonText="Autorisierungs-Token abrufen"
			subtype="success"
			@buttonClicked="sendSystemCommand('retrieveMSALTokens', {})"
		>
			<template #help>
				Zugangstoken wird abgerufen und gespeichert, damit das Backup
				durchgeführt werden kann. Zugangstoken werden nur lokal auf
				dieser openWB gespeichert.
			</template>
		</openwb-base-button-input>
	</div>
</template>
