<template>
	<div class="backup-cloud-onedrive">
		...
		>
			<template #help>
				Diese URL im Browser öffnen und den Code eingeben.
			</template>
		</openwb-base-text-input>
		<openwb-base-text-input
			title="3. Diesen Code eingeben und Zugang authorisieren"
			subtype="text"
			readonly
		...
		<openwb-base-textarea
			...
			subtype="text"
			readonly
			:model-value="configuration.persistent_tokencache"
			@update:model-value="
				updateConfiguration($event,
					'configuration.persistent_tokencache'
				)
			"
		/>
	</div>
