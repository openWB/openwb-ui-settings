<template>
	<div class="electricity-tariff-energycharts">
		...
		<openwb-base-number-input
			...
			@update:model-value="
				updateConfiguration(
					parseFloat($event.toFixed(3)),
					'configuration.surcharge',
				)
			"
		>
			<template #help>
				Die Auswahl (Energy-Charts) kann für alle Stromanbieter
				ausgewählt werden, die die Börsenstrompreise in einem
				variablen/dynamischen Tarif an die Kunden weitergeben. Das Modul
				soll es ermöglichen, dass zu tagesaktuellen Strompreisen unter
				Berücksichtigung eines Aufschlags zu günstigen Zeiten
				automatisch geladen werden kann und somit die Preisschwankungen
				an der Börse optimal ausgenutzt werden.
			</template>
		</openwb-base-number-input>
	</div>
</template>
