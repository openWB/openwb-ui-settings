	...
	<div class="mqtt-bridge-configuration">
		<openwb-base-card title="Konfigurierte MQTT-Brücken">
			...
			>
				...
				>
					<template #help>
						Die Bezeichnung darf nur aus Buchstaben ohne Umlaute,
				...
				<openwb-base-number-input
					...
					@update:model-value="
						updateState(mqttBridgeKey, $event, 'remote.port')
					"
				/>
				<openwb-base-text-input
					title="Benutzername"
				...
				<openwb-base-text-input
					...
					@update:model-value="
						updateState(mqttBridgeKey, $event, 'remote.prefix')
					"
				>
					<template #help>
						Das Präfix darf nur aus Buchstaben ohne Umlaute, Zahlen
						und Bindestrichen bestehen. Optional kann mit einem
						Schrägstrich "/" eine Topic-Struktur abgebildet werden.
					</template>
				</openwb-base-text-input>
				<openwb-base-text-input
					title="Client ID"
					...
					@update:model-value="
						updateState(mqttBridgeKey, $event, 'remote.client_id')
					"
				/>
				<openwb-base-button-group-input
					...
					@update:model-value="
						updateState(mqttBridgeKey, $event, 'remote.protocol')
					"
				/>
				<openwb-base-button-group-input
					title="TLS Version"
					...
					@update:model-value="
						updateState(mqttBridgeKey, $event, 'remote.tls_version')
					"
				/>
				<openwb-base-button-group-input
					...
					@update:model-value="
						updateState(mqttBridgeKey, $event, 'remote.try_private')
					"
				>
					<template #help>
						Aktiviert eine proprietäre Protokoll-Erweiterung des
						Mosquitto Brokers, welche dem entfernten Broker
						signalisiert dass es sich um eine MQTT Brücke handelt.
						Ergibt bessere Leistung mit Mosquitto-Brokern, ist
						jedoch inkompatibel mit vielen anderen MQTT-Brokern.
						Daher bitte nur aktivieren, wenn der Ziel-Broker sicher
						ein Mosquitto-Broker ist.
					</template>
				</openwb-base-button-group-input>
				<hr />
...
export default {
	...
	methods: {
		...
		removeMqttBridgeModal(mqttBridge, event) {
			...
			this.modalMqttBridgeIndex = parseInt(
				mqttBridge.match(/(?:\/)(\d+)$/)[1]
			);
