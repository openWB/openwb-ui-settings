...
const datasetTemplates = {
  ...
  "cp-energy_imported_bat": {
    label: "Ladepunkt (Speicher-Anteil)",
    category: "chargepoint",
    unit: "kWh",
    type: "bar",
    jsonKey: null,
  ...
  "hc-energy_imported_bat": {
    label: "Hausverbrauch (Speicher-Anteil)",
    category: "component",
    unit: "kWh",
    type: "bar",
    jsonKey: null,
...
export default {
  ...
  data() {
    return {
      ...
      mqttTopics: [
        ...
        { topic: "openWB/log/monthly/#", writeable: false },
        { topic: "openWB/log/yearly/#", writeable: false },
  ...
  computed: {
    ...
    chartTotals() {
      if (this.$store.state.mqtt[this.baseTopic + this.commandData.date]) {
        if (
          ...
          var totals = structuredClone(toRaw(this.$store.state.mqtt[this.baseTopic + this.commandData.date].totals));
    ...
    chartDataObject() {
      const convertMeasurementValues = (row) => {
      ...
      if (this.$store.state.mqtt[this.baseTopic + this.commandData.date]) {
        ...
        let myData = structuredClone(toRaw(chartEntries));
        myData.forEach((row) => {
          convertMeasurementValues(row);
        });
        return myData;
      }
      return undefined;
      ...
      if (this.chartDataObject) {
        Object.entries(this.chartDataStructure).forEach(([baseObject, objects]) => {
          ...
          if (objectKeys.includes("all")) {
            ...
            if (["bat", "pv", "consumer"].includes(baseObject) && objectKeys.length <= 2) {
  ...
  methods: {
    ...
    addDataset(baseObject, objectKey, elementKey, datasetKey) {
      ...
      if (datasetTemplates[datasetTemplate]) {
        var newDataset = structuredClone(toRaw(datasetTemplates[datasetTemplate]));
        newDataset.parsing.yAxisKey = datasetKey;
        newDataset.jsonKey = datasetKey;
        newDataset.data = this.chartDataObject;
        newDataset.category = datasetTemplates[datasetTemplate].category;
        // build dataset label
